      ...
      <form className="card" onSubmit={submitDcr} style={{ display:"flex", flexDirection:"column", gap:18 }}>
        ...
        <div className="field">
          ...
          <div className="field" style={{ margin:"10px 0 0" }}>
            ...
            {gpsStatus && gpsStatus !== "Location captured." && (
              <p className="muted" style={{ margin:"4px 0 0" }}>{gpsStatus}</p>
            )}
          </div>
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10, marginTop:8 }}>
            <div className="field" style={{ margin:0 }}>
              ...
              <select value={prescriptionInterest} onChange={e => setPrescriptionInterest(e.target.value as typeof prescriptionInterest)}>
                {PRESCRIPTION_INTEREST_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <div className="field" style={{ margin:0 }}>
          ...
          {hasJointWork && (
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginTop:10, padding:12, background:"var(--panel-strong)", borderRadius:8 }}>
              <div className="field">
                ...
                {managers.length > 0 ? (
                  <select value={jointManager} onChange={e => setJointManager(e.target.value)}>
